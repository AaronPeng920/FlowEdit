# Copied and revised from diffusers.pipelines.pipeline_stable_diffusion_3.py
...
class FlowEditPipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin):
    ...
    # Copied from diffusers.pipelines.stable_diffusion_3.StableDiffusion3Pipeline._get_t5_prompt_embeds()
    ...
    # Copied from diffusers.pipelines.stable_diffusion_3.StableDiffusion3Pipeline._get_clip_prompt_embeds()
    ...
    @property
    def source_guidance_scale(self):
        return self._source_guidance_scale
    ...
    def __call__(
        ...
        self.check_inputs(
            ...
            negative_prompt_2=source_negative_prompt_2,
            negative_prompt_3=source_negative_prompt_3,
        ...
        self._source_guidance_scale = source_guidance_scale
        ...
        (
            prompt_embeds,                                  # [B, 333, 4096] or [B, 154, 4096]
        ...
        (
            source_prompt_embeds,                                  # [B, 333, 4096] or [B, 154, 4096]
            ...
            prompt_3=source_prompt_3,
            negative_prompt=source_negative_prompt,
            negative_prompt_2=source_negative_prompt_2,
            negative_prompt_3=source_negative_prompt_3,
            do_classifier_free_guidance=self.source_do_classifier_free_guidance,
            prompt_embeds=source_prompt_embeds,
            negative_prompt_embeds=source_negative_prompt_embeds,
            pooled_prompt_embeds=source_pooled_prompt_embeds,
            negative_pooled_prompt_embeds=source_negative_pooled_prompt_embeds,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.transformer(
                    hidden_states=latent_model_input,                       # z_t
                    timestep=timestep,                                      # t
                    encoder_hidden_states=prompt_embeds,                    # c
                    pooled_projections=pooled_prompt_embeds,                # y
                    joint_attention_kwargs=self.joint_attention_kwargs,
                    return_dict=False,
                )[0]        # [B, 16, H / 8 (128), W / 8 (128)] 

                # perform guidance
                if self.do_classifier_free_guidance:
                    noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                    target_noise_pred_uncond, source_noise_pred_uncond = noise_pred_uncond.chunk(2, dim=0)
                    target_noise_pred_text, source_noise_pred_text = noise_pred_text.chunk(2, dim=0)
                    target_noise_pred = target_noise_pred_uncond + self.guidance_scale * (target_noise_pred_text - target_noise_pred_uncond)
                    source_noise_pred = source_noise_pred_uncond + self.source_guidance_scale * (source_noise_pred_text - source_noise_pred_uncond)
                    noise_pred = torch.cat([target_noise_pred, source_noise_pred], dim=0)
                ...
                if latents.dtype != latents_dtype:
                    if torch.backends.mps.is_available():
                        ...
                        source_latents = source_latents.to(latents_dtype)
                ...
                if callback_on_step_end is not None:
                    ...
                    source_latents = callback_outputs.pop("source_latents", source_latents)
        ...
        if output_type == "latent":
            image = latents
            if return_source:
                source_image = source_latents
        else:
            latents = (latents / self.vae.config.scaling_factor) + self.vae.config.shift_factor
            image = self.vae.decode(latents, return_dict=False)[0]
            image = self.image_processor.postprocess(image, output_type=output_type)
            if return_source:
                source_latents = (source_latents / self.vae.config.scaling_factor) + self.vae.config.shift_factor
                source_image = self.vae.decode(source_latents, return_dict=False)[0]
