# Copied from https://github.com/cure-lab/PnPInversion/blob/main/evaluation/evaluate.py
...
def calculate_metric(
    ...
    if metric == "structure_distance":
        return metrics_calculator.calculate_structure_distance(
            src_image, tgt_image, None, None
        )
    if metric == "psnr_unedit_part":
        if (1 - src_mask).sum() == 0 or (1 - tgt_mask).sum() == 0:
            return "nan"
        else:
            return metrics_calculator.calculate_psnr(
    ...
    if metric == "lpips_unedit_part":
        if (1 - src_mask).sum() == 0 or (1 - tgt_mask).sum() == 0:
            ...
            return metrics_calculator.calculate_lpips(
                src_image, tgt_image, 1 - src_mask, 1 - tgt_mask
            )
    if metric == "mse_unedit_part":
        if (1 - src_mask).sum() == 0 or (1 - tgt_mask).sum() == 0:
            ...
            return metrics_calculator.calculate_mse(
                src_image, tgt_image, 1 - src_mask, 1 - tgt_mask
            )
    if metric == "ssim_unedit_part":
        if (1 - src_mask).sum() == 0 or (1 - tgt_mask).sum() == 0:
            ...
            return metrics_calculator.calculate_ssim(
                src_image, tgt_image, 1 - src_mask, 1 - tgt_mask
            )
...
all_tgt_image_folders = {
    "flowedit": "flowedit_result/annotation_images",      ##**##
}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--annotation_mapping_file", type=str, default="datasets/PIE-Bench/PIE-Bench_v1/mapping_file.json"
    )
    parser.add_argument(
        "--metrics",
        nargs="+",
    ...
    parser.add_argument(
        "--src_image_folder", type=str, default="datasets/PIE-Bench/PIE-Bench_v1/annotation_images"
    )
    parser.add_argument(
        "--tgt_methods",
        nargs="+",
    ...
    parser.add_argument(
        "--evaluate_whole_table", action="store_true"
