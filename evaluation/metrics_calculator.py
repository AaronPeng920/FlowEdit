# Copied from https://github.com/cure-lab/PnPInversion/blob/main/evaluation/matrics_calculator.py
...
class MetricsCalculator:
    def __init__(self, device) -> None:
        ...
        self.clip_metric_calculator = CLIPScore(
            model_name_or_path="/data/pengzhengwei/checkpoints/openai/clip-vit-large-patch14"
        ).to(device)
    ...
    def calculate_structure_distance(
        self, img_pred, img_gt, mask_pred=None, mask_gt=None, use_gpu=True
    ):
        ...
        img_pred = torch.from_numpy(np.transpose(img_pred, axes=(2, 0, 1))).to(
            self.device
        )
