# Copied and revised from https://github.com/sled-group/InfEdit/blob/main/seq_aligner.py
...
def global_align(x, y, score):
    ...
    # Calculate site scores and traceback
...
def get_aligned_sequences(x, y, trace_back):
    ...
    i = len(x)
    j = len(y)
    mapper_y_to_x = []      # two-tuple (yi, xi) indicates yi-th target token mapping to xi-th source token, if xi is -1, then the source token is absent.
    while i > 0 or j > 0:
        # diag, match
        if trace_back[i, j] == 3:
            x_seq.append(x[i-1])
        ...
        # left, mismatch, source(dim=0) token absence
        elif trace_back[i][j] == 1:
            x_seq.append('-')
            y_seq.append(y[j-1])
            j = j-1
            mapper_y_to_x.append((j, -1))
        # up, mismatch, target(dim=1) token absence
...
def get_mapper(x: str, y: str, specifier, tokenizer, encoder, device, max_len=77):
    ...
    x_seq_inds, y_seq_inds, mapper_base = get_aligned_sequences(x_seq, y_seq, trace_back)
    
    x_seq_string = [tokenizer.decode(ind) if ind != '-' else ind for ind in x_seq_inds]
    y_seq_string = [tokenizer.decode(ind) if ind != '-' else ind for ind in y_seq_inds]
    x_seq_string = x_seq_string[::-1]
    y_seq_string = y_seq_string[::-1]
    ...
    original_mapper = -1 * torch.ones(max_len, dtype=torch.int64)
    original_mapper[:mapper_base.shape[0]] = mapper_base[:, 1]
    original_mapper[mapper_base.shape[0]:] = len(y_seq) + torch.arange(max_len - len(y_seq))
    ...
    while i<len(y_seq):
        ...
        if alphas[i] == 0:
            start = i
            # find the longest gap sequence of target [start, i)
            while alphas[i] == 0:
                i += 1
            max_sim = float('-inf')
            max_s = None
            max_t = None
            # gap sequence pair between target and source 
    ...
    return mapper, alphas, m, alpha_e, alpha_m, (x_seq_string, y_seq_string, original_mapper)


def get_refinement_mapper(prompts, specifiers, tokenizer, encoder, device, max_len=77):
    x_seq = prompts[0]
    mappers, alphas, ms, alpha_objs, alpha_descs  = [], [], [], [], []
    x_seq_strings, y_seq_strings, original_mappers = [], [], []
    for i in range(1, len(prompts)):
        mapper, alpha, m, alpha_obj, alpha_desc, (x_seq_string, y_seq_string, original_mapper) = get_mapper(x_seq, prompts[i], specifiers[i-1], tokenizer, encoder, device, max_len)
        ...
        x_seq_strings.append(x_seq_string)
        y_seq_strings.append(y_seq_string)
        original_mappers.append(original_mapper)
    return torch.stack(mappers), torch.stack(alphas), torch.stack(ms),  torch.stack(alpha_objs), torch.stack(alpha_descs), (x_seq_strings, y_seq_strings, torch.stack(original_mappers))
